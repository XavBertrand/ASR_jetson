...
DEFAULT_MODEL = "flaubert/flaubert_base_cased"
...
def _detect_simple_names(text: str) -> List[Span]:
    """Détecte les noms propres simples après des mots-clés de contexte."""
    spans = []

    # Chercher : (chez|avec|et|puis) + NomPropre
    pattern = r'\b(chez|avec|et|puis)\s+([A-ZÉÈÀÂÎÔÙÛÄËÏÖÜÇ][a-zàâäéèêëïîôùûüÿœæç]{3,11}(?:\s+[A-ZÉÈÀÂÎÔÙÛÄËÏÖÜÇ][a-zàâäéèêëïîôùûüÿœæç]{3,11})?)\b'

    BLACKLIST = {'Toulouse', 'Paris', 'Section', 'Email', 'France'}
    ...
    for m in re.finditer(pattern, text, re.IGNORECASE):
        ...
        # Position du nom (pas du mot-clé)
        name_start = m.start(2)
        name_end = m.end(2)

        spans.append(Span(name, name_start, name_end, "PER", "regex_name", 45))
...
def _detect_ner_spans_chunked(text: str, ner_pipe: Any, max_chars: int = 2000, overlap: int = 80) -> List["Span"]:
    ...
    while start < L:
        ...
        for e in out:
            ...
            if re.search(r'<(NOM|ORG|LIEU|EMAIL|TEL|CAT)_\d+>', mention):
                continue
...
def _post_label_corrections(spans, text: str):
    ...
    # Indices forts d'organisation (évite de confondre avec une personne)
    ORG_HINTS = re.compile(
        r"\b(avocat|avocats|cabinet|groupe|sarl|sas|association|udaf|airbus|fd&s|holding|banque)\b",
        flags=re.I
    )
    ...
    def looks_like_person(t: str) -> bool:
        # Si ça ressemble explicitement à une organisation, ce n'est PAS une personne.
        if ORG_HINTS.search(t):
            return False
    ...
    for s in spans:
        ...
        # 2) "chez X" -> vérifier si c'est plutôt une personne
        ctx_start = max(0, s.start - 10)
        context = text[ctx_start:s.start].lower()
        if "chez" in context:
            if s.label in ("PER", "CAT"):
                # Si looks_like_person, garder PER
        ...
        # 6) CAT : garder ceux qui ressemblent à des personnes
        if s.label == "CAT":
            # Nettoyage
            t2 = clean_person(t)
            if looks_like_person(t2):
                # Convertir CAT -> PER si c'est clairement un nom
                s = Span(t2, s.start, s.end, "PER", s.source, s.priority)
                corrected.append(s)
            continue  # sinon jette
...
def group_person_aliases(
    ...
    # Filtre strict : n'accepter des CAT que si ça "ressemble" à une personne,
    # et exclure tout ce qui a des indices d'organisation (avocats, groupe, sas, sarl, association, airbus, etc.).
    ORG_HINTS = re.compile(
        r"\b(avocat|avocats|cabinet|groupe|sarl|sas|sasu|eurl|sa|association|udaf|airbus|fd&s|holding|banque|universit[eé]|h[oô]pital|mairie|minist[eè]re)\b",
        flags=re.I
    )
    def cat_looks_like_person(t: str) -> bool:
        if ORG_HINTS.search(t):
            return False
        # 1–2 mots en "TitleCase" (ou "M. Nom") = patron de personne
        return re.fullmatch(
            r"(?:M\.\s+)?[A-ZÉÈÀÂÎÔÙÛÄËÏÖÜÇ][\w\-’']+(?:\s+[A-ZÉÈÀÂÎÔÙÛÄËÏÖÜÇ][\w\-’']+)?",
            t.strip()
        ) is not None

    if include_cat_as_person:
        idxs = [
            i for i, s in enumerate(spans)
            if (s.label == "PER") or (s.label == "CAT" and cat_looks_like_person(s.text))
        ]
    else:
        idxs = [i for i, s in enumerate(spans) if s.label == "PER"]
...
def _dedupe_and_retag_mapping(mapping: dict) -> dict:
    ...
    - Priorité de type: PERSON > ORG > LIEU > EMAIL > TEL > CAT.
    - Recrée des tags cohérents avec le type (NOM_i / ORG_i / LIEU_i / EMAIL_i / TEL_i / CAT_i).
    """
    type_rank = {"PERSON": 5, "ORG": 4, "LIEU": 3, "EMAIL": 2, "TEL": 1, "CAT": 0}
    ...
    # 2) re-tagger proprement: NOM_i / ORG_i / LIEU_i / EMAIL_i / TEL_i / CAT_i
    counters = {"PERSON": 0, "ORG": 0, "LIEU": 0, "EMAIL": 0, "TEL": 0, "CAT": 0}
    type_prefix = {"PERSON": "NOM", "ORG": "ORG", "LIEU": "LIEU", "EMAIL": "EMAIL", "TEL": "TEL", "CAT": "CAT"}
    new_map = {}
    for item in merged:
        t = item["type"]
        counters[t] += 1
...
class Anonymizer:
    ...
    def anonymize(self, text: str,
        ...
        spans += _detect_simple_names(text)
        spans += _detect_ner_spans_chunked(text, self.ner)
        print(f"DEBUG NER spans: {[(s.text, s.label) for s in spans if s.source == 'ner']}")
