...
## Document Anonymization CLI (`asr anonymize`)

Secure document anonymization is opt-in and does not alter default audio pipeline behavior.

```bash
uv run asr anonymize \
  --input tests/data/anonymization/fixtures/us1 \
  --output outputs/anonymization_demo \
  --case-id CASE-2026-0001 \
  --policy strict_offline \
  --report outputs/anonymization_demo/report.json
```

Security/runtime defaults:
- no network by default (`strict_offline`)
- sanitized diagnostics/logs (no raw PII snippets)
- resilient batch mode (one failed doc does not stop the whole batch)
- restricted permissions on sensitive artifacts (`0700` directories, `0600` files)

Determinism and mappings:
- placeholders are deterministic within one `case_id`
- placeholders are isolated across different `case_id`
- reversible mappings are encrypted at rest (AEAD) when emitted

Mapping key configuration:
```bash
export ANON_KEY_PROVIDER=env
export ANON_KEY_ID=anon-key-v1
export ANON_MAPPING_KEY="<base64-aes-key>"
```

Internal mapping authorization boundary:
- internal resolution requires `X-Internal-API-Key` equivalent (`--internal-api-key` in CLI path)
- missing/invalid credential returns sanitized denial

---

## Docker Anonymization
...
Run anonymization CLI in container (standard or Jetson image):
```bash
