...
def test_transformer_anonymization_basic():
    ...
    assert "Marine" in restored
    assert "Delphine" in restored
    assert "Super Avocats" in restored
...
def test_transformer_with_domain_entities():
    ...
    domain_entities = {
        ...
        "ORGANIZATION": ["Super Avocats", "CJD", "UDAF", "Mille Mécat"]
    }

    text = ("Marine a rappelé le client du CJD pour le compte du cabinet Super Avocats. "
            "Delphine devait valider la rupture conventionnelle.")
    ...
    assert "Marine" not in anon_text, "Marine devrait être anonymisé"
    assert "Delphine" not in anon_text, "Delphine devrait être anonymisé"
    assert "Super Avocats" not in anon_text, "Super Avocats devrait être anonymisé"
    ...
    # Vérifie qu'Super Avocats est bien classé en ORGANIZATION
    found_org = False
    for tag, info in mapping["entities"].items():
        values_lower = [v.lower() for v in info["values"]]
        if "super avocats" in values_lower:
            assert info["label"] == "ORGANIZATION", f"Super Avocats devrait être ORG, pas {info['label']}"
            found_org = True
        if "cjd" in values_lower:
            assert info["label"] == "ORGANIZATION", f"CJD devrait être ORG, pas {info['label']}"

    assert found_org, "Super Avocats devrait être détecté comme ORGANIZATION"
...
SPEAKER_2 : Et sur le fonctionnement avec Marine?
...
SPEAKER_2 : marine a aussi et que vous avez que moi je commence à avoir aussi et c'est ça qui à moi c'est ça qui nous fait vivre de manière plus facile en ce moment.
...
SPEAKER_2 : Il y a des informations qui vous manquent. L'augmentation marine, c'est qu'elle devrait, et la classification m'imposerait, de la passer au coefficient 350. Sauf que le cap entre 300 et 350 en termes de rémunération. . . step était énorme, vraiment énorme, c'était plus de 500 euros brut par mois et donc je l'ai reçu, je lui ai dit je sais que tu mérites ça, je sais que tu mérites les 350, sauf que la situation fait que je ne peux pas le faire et donc je lui ai proposé ce qu'elle a accepté. que je le fasse en deux temps donc je l'ai augmenté sans la faire passer au coefficient 350 je l'ai augmenté de la moitié du step en lui disant dès que je peux faire le tranche supplémentaire je le fais de toute façon dans un an je crois que c'est en janvier en janvier 2026 je ferai le step supplémentaire pour la passer à 350 parce qu'en fin de compte son son et puis il faut que je fasse juriste junior il faut que je fasse sauter mais voilà pourquoi elle a eu une augmentation et je m'étais dit au ce moment là en janvier bouge en janvier j'avais dit je fais ça pour marine parce que là elle voudrait me faire elle pourrait m'obliger à le payer le reste elle a compris aussi la situation et elle a accepté le fait que je fasse en deux temps mais n'empêche que donc ça c'était des des informations et je m'excuse de ne pas vous les avoir données parce que je ne me suis pas mis à votre place et j'aurais pu me dire, mais elle ne va pas comprendre, Jacqueline, pourquoi j'augmente Marine et que je ne lui fais rien pour elle. Et dans ma tête, deuxième information que je voulais vous donner, dans ma tête, quand j'ai fait ça, quand j'ai eu cette conversation avec Marine et que j'ai décidé ça, Je me suis dit, pour Jacqueline, je vais le faire plutôt une sorte de prime Macron ou quelque chose comme ça. Qui dit prime Macron, dit prime Macron aussi pour Marine. Donc, j'étais un petit peu bloquée. Et puis, en plus, je voulais voir comment ça se passait l'année. J'ai toujours extrêmement peur. mettre dans une situation compliquée mais bon ça c'est des explications ça n'est aucunement des justifications
...
SPEAKER_2 : Ça c'est chouette. Qu'est-ce qui contribue le plus à une bonne ambiance selon vous? Le respect, la bonne entente, l'écoute, l'entrée, l'esprit d'Akima. Si on est honnête. Je crois que. . . Peut-être un ou deux mots près, je crois que vous avez eu exactement la même réponse que Marine.
...
SPEAKER_2 : Ce qui me faisait peur avec le. . . Et là, encore une fois, je l'ai vu avec ma propre perception, le fait de cette manivelle, j'ai dit, mais Jacqueline, elle ne prendra jamais le temps de me lever et de me baisser. C'est ce que je vois, donc je suis contente.
...
def test_transformer_merges_similar_variants():
    ...
    text = "Marine a parlé à marines. PENNYLANE rencontre Pény Lène."
    anonymizer = TransformerAnonymizer()

    marine_start = text.index("Marine")
    marines_start = text.index("marines")
    penny_start = text.index("PENNYLANE")
    peny_start = text.index("Pény Lène")

    entities = [
        {"start": marine_start, "end": marine_start + len("Marine"), "entity_type": "PERSON", "score": 0.95},
        {"start": marines_start, "end": marines_start + len("marines"), "entity_type": "PERSON", "score": 0.80},
...
def test_deanonymization_exact():
    ...
    original = "Marine et Delphine travaillent chez Super Avocats à Montpellier."

    anonymizer = TransformerAnonymizer(
        domain_entities={
            "PERSON": ["Marine", "Delphine"],
            "ORGANIZATION": ["Super Avocats"]
        }
    )
...
def test_helper_function():
    ...
    text = "Test avec Marine et Super Avocats"
    domain = {"PERSON": ["Marine"], "ORGANIZATION": ["Super Avocats"]}
